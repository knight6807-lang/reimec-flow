...
function getEffectiveBillingPeriodEnd(ws: WorkspaceRecord) {
  const candidates = [ws.currentPeriodEnd ?? 0, ws.manualBillingPaidUntil ?? 0].filter((value) => Number.isFinite(value) && value > 0);
...
function getWorkspaceBillingStatus(ws: WorkspaceRecord, userId?: string) {
  ...
  if ((ws.manualBillingPaidUntil ?? 0) > now) return "manual";
  return ws.subscriptionStatus ?? null;
}

function workspaceHasActiveSubscription(ws: WorkspaceRecord) {
  const now = nowUnixSeconds();
  if ((ws.manualBillingPaidUntil ?? 0) > now) {
    return true;
  }
  const status = (ws.subscriptionStatus ?? "").toLowerCase();
  if ((status === "active" || status === "trialing") && (!(ws.currentPeriodEnd ?? 0) || (ws.currentPeriodEnd ?? 0) > now)) {
    return true;
  }
  return false;
}
...
function workspaceHasBillingAccess(ws: WorkspaceRecord, userId?: string) {
  ...
  return userHasManualBillingAccess(user) || workspaceHasActiveSubscription(ws);
}
...
function sendBillingRequired(res: express.Response, ws: WorkspaceRecord) {
  res.status(402).json({
    ...
    billing: {
      status: getWorkspaceBillingStatus(ws),
...
function sendLoginBillingRequired(res: express.Response, ws: WorkspaceRecord) {
  res.status(402).json({
    error: "payment required before login",
    workspaceId: ws.id,
    billing: {
      status: getWorkspaceBillingStatus(ws),
      currentPeriodEnd: getEffectiveBillingPeriodEnd(ws),
...
function getEmailModuleRoot() {
  const root = path.join(getJobnestRoot(), "System", "Email");
  ensureDir(root);
  return root;
}
...
function getPartDnaModuleRoot() {
  const root = path.join(getJobnestRoot(), "System", "Part DNA");
  ensureDir(root);
  return root;
}
...
function getSmartJobQueueModuleRoot() {
  const root = path.join(getJobnestRoot(), "System", "Smart Job Queue");
  ensureDir(root);
  return root;
}
...
function getStockMaterialModuleRoot() {
  const root = path.join(getJobnestRoot(), "System", "Stock Material");
  ensureDir(root);
  return root;
}
...
app.post("/api/auth/login", (req, res) => {
  ...
  ensureUserAccountRef(user);
  const blockedWorkspace = getBillingBlockedWorkspace(user.id);
  if (blockedWorkspace) {
    sendLoginBillingRequired(res, blockedWorkspace);
    return;
  }
  const token = crypto.randomBytes(24).toString("hex");
  const session: SessionRecord = { token, userId: user.id, createdAt: new Date().toISOString() };
...
function setupFileActivityWatcher() {
  ...
  try {
    fs.watch(root, { recursive: true }, (_eventType, rawFileName) => {
...
app.post("/api/stock/use", (req, res) => {
  try {
    markStockUsed(getStockMaterialDb(), String(req.body?.jobId ?? "").trim());
    res.json({ ok: true });
  } catch (error) {
