...
type AdminSubscriptionCompanyRow = BillingCompanyRecord & {
...
type AdminSubscriptionDashboard = {
...
type UserSummary = { id: string; email: string; name?: string; accountRef?: string };
...
type CustomerRecord = { id: string; name: string; email?: string; phone?: string; address?: string; notes?: string };
...
type InvoiceDocRecord = BusinessDocRecord & {
  source: "quote" | "manual";
  quoteId?: string;
...
type PartDnaPreviousPricePopup = {
  entries: Array<{
    partName: string;
    partCode: string;
...
type OffcutBrainIntelligenceRecord = {
  id: number;
  offcutId: number;
  material: string;
  thickness: number;
...
type OffcutBrainRecommendation = {
...
type BrainRecommendationRecord = {
  id: number;
  moduleName: string;
  recommendationType: string;
  title: string;
...
type InboxAttachmentMeta = { part: string; name: string; contentType: string; sizeBytes?: number };
...
type ParsedPurchaseOrderInsights = {
...
const AUTH_PASSWORD_KEY = "authPassword";
const AUTH_PASSWORD_STORE_KEY = "authPassword";
const CLOUD_DEVICE_TOKEN_STORE_PREFIX = "cloudDeviceToken";
const APP_OWNER_EMAIL = "knight6807@gmail.com";
const EMAIL_OAUTH_STORE_PREFIX = "msgraph";
const SUBSCRIPTION_BANK_NAME = "Capitec";
const SUBSCRIPTION_ACCOUNT_NUMBER = "1607030940";

function deriveAccountRef(email: string) {
...
const JOB_DXF_THICKNESS_OPTIONS = [0.9, 1.2, 1.5, 1.6, 2, 3, 4, 4.5, 5, 6, 8, 10, 12, 15, 20, 25, 30];
const APP_FEATURE_OPTIONS: Array<{ id: AppFeatureId; label: string }> = [
  { id: "chat", label: "Chat" },
  { id: "jobs", label: "Jobs" },
...
const BRAND_LOGO_SRC = `${import.meta.env.BASE_URL}qouterx-logo-v2.png`;

function formatMachineLabelFromKey(key: string) {
  return key
...
async function pickAccentFromLogo(dataUrl: string) {
...
type DxfBounds = { minX: number; minY: number; maxX: number; maxY: number };
type DxfReaderPartPreview = {
  id: string;
  name: string;
  layer: string;
  quantity: number;
...
type NestingPlateLayout = { placements: NestingPlacement[] };
type NestingResult = {
  plateWidthMm: number;
...
function parseDxfSegments(raw: string): DxfSegment[] {
...
function getDxfBounds(segments: DxfSegment[]): DxfBounds | undefined {
...
function createSegmentThumbnailDataUrl(
  segments: DxfSegment[],
  size = 96,
  background = "#0b1220",
  stroke = "#93c5fd"
...
function createSegmentSvgDataUrl(segments: DxfSegment[], size = 700, background = "#ffffff", stroke = "#0f172a") {
  const bounds = getDxfBounds(segments);
  if (!bounds) return undefined;
  const width = Math.max(1, bounds.maxX - bounds.minX);
  const height = Math.max(1, bounds.maxY - bounds.minY);
...
function createRectangleSegments(widthMm: number, heightMm: number, layer: string, entityPrefix: string, offsetX = 0, offsetY = 0) {
...
function createCircleSegments(diameterMm: number, layer: string, entityPrefix: string, offsetX = 0, offsetY = 0, steps = 72) {
...
function createRegularPolygonSegments(
  sides: number,
  radiusMm: number,
  layer: string,
  entityPrefix: string,
...
function createSlotSegments(lengthMm: number, widthMm: number, layer: string, entityPrefix: string, offsetX = 0, offsetY = 0, steps = 20) {
...
function buildDxfFromSegments(segments: DxfSegment[]) {
  const lines = segments
    .map(
      (segment) =>
        `0\nLINE\n8\n${segment.layer}\n10\n${segment.x1}\n20\n${segment.y1}\n30\n0\n11\n${segment.x2}\n21\n${segment.y2}\n31\n0\n`
...
function mergeContainedComponents(components: DxfSegment[][]) {
...
function groupComponentsByDrawingIslands(components: DxfSegment[][]) {
  if (components.length <= 1) return components;
...
function boundsDistanceMm(a?: DxfBounds, b?: DxfBounds) {
...
function endpointDistanceMm(a: DxfSegment[], b: DxfSegment[]) {
...
function mergeComponentsByProximity(components: DxfSegment[][], thresholdMm: number) {
  if (components.length <= 1) return components;
  const parent = components.map((_c, i) => i);
...
function getPartSignature(segments: DxfSegment[]) {
...
function estimateNestingForPlate(
...
async function readDxfText(file: File) {
  const buf = await file.arrayBuffer();
  return decodeDxfArrayBuffer(buf);
}

function decodeDxfArrayBuffer(buf: ArrayBuffer) {
...
function createDxfThumbnailDataUrl(raw: string) {
...
function UpdateStatusCard(props: {
